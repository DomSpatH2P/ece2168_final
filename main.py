...
def describe_motion(motion): #determine plane of motion; actual motion depends on whether background or obejct is darker
    ...
    if abs(motion[0]) >  DISP_THRESH: 
        return "⇑⇓" #north/south
    if abs(motion[1]) >  DISP_THRESH:
        return "⇐⇒" #east/west
    return None
...
while True:
    ...
    if(NEUROMORPHIC):
        ...
        on_mask = (on_core | on_border | on_b)
        on_centroids = get_clusters(on_mask,MIN_SAMPLES)
        off_core, off_border, off_b = dbscan.run(off_pts)
        off_mask = (off_core | off_border | off_b)
        off_centroids = get_clusters(off_mask,MIN_SAMPLES)
    ...
    oldgray = gray
    t += 1
    average = average*(t-1)/t + (end-start)/t

cam.release()
cv2.destroyAllWindows()
print(f"Neuromorphic ({NEUROMORPHIC}); eps = {EPS}; average period {average} s")